        ...
        rl.question('Enter your PocketBase Admin Password: ', async (password) => {
            try {
                ...
                const desiredFields = [
                    ...
                    { name: 'city_lat', type: 'number', required: false },
                    { name: 'city_lng', type: 'number', required: false },
                    ...
                    { name: 'last_profile_edit', type: 'date', required: false },
                ...
                await createCollectionSafe({
                    ...
                    fields: [
                        { name: 'user', type: 'relation', required: true, collectionId: usersCollection.id, cascadeDelete: true, maxSelect: 1 },
                        { name: 'type', type: 'select', required: true, maxSelect: 1, values: ["match", "verification_approved", "verification_rejected", "admin_warning", "system", "aviary_warning", "aviary_update"] },
                ...
                await createCollectionSafe({
                    name: 'bird_deliveries',
                    type: 'base',
                    listRule: "@request.auth.id = sender || @request.auth.id = recipient",
                    viewRule: "@request.auth.id = sender || @request.auth.id = recipient",
                    createRule: "@request.auth.id != ''",
                    updateRule: null,
                    deleteRule: null,
                    fields: [
                        { name: 'sender', type: 'relation', required: true, collectionId: usersCollection.id, cascadeDelete: true, maxSelect: 1 },
                        { name: 'recipient', type: 'relation', required: true, collectionId: usersCollection.id, cascadeDelete: true, maxSelect: 1 },
                        { name: 'bird_type', type: 'select', required: true, maxSelect: 1, values: ["raven", "owl", "albatross"] },
                        { name: 'origin_city', type: 'text', required: true },
                        { name: 'origin_coords', type: 'json', required: false },
                        { name: 'dest_city', type: 'text', required: true },
                        { name: 'dest_coords', type: 'json', required: false },
                        { name: 'departed_at', type: 'date', required: true },
                        { name: 'estimated_arrival', type: 'date', required: true },
                        { name: 'is_night_locked', type: 'bool', required: false },
                        { name: 'burn_on_crash', type: 'bool', required: false },
                        { name: 'letter_original', type: 'text', required: true, options: { max: 1000 } },
                        { name: 'letter_smudged', type: 'text', required: false, options: { max: 1500 } },
                        { name: 'status', type: 'select', required: true, maxSelect: 1, values: ["flying", "delivered", "crashed", "rescued", "degraded"] },
                        { name: 'crash_progress', type: 'number', required: false },
                        { name: 'crash_coords', type: 'json', required: false },
                        { name: 'crash_reason', type: 'text', required: false },
                        { name: 'flight_events', type: 'json', required: false },
                        { name: 'rescue_count', type: 'number', required: false },
                        { name: 'rescue_target', type: 'number', required: false },
                        { name: 'rescued_by', type: 'relation', required: false, collectionId: usersCollection.id, maxSelect: 1000 },
                        { name: 'warning_notified', type: 'bool', required: false },
                        { name: 'crash_notified', type: 'bool', required: false },
                        { name: 'created', type: 'autodate', onCreate: true, onUpdate: false },
                        { name: 'updated', type: 'autodate', onCreate: true, onUpdate: true }
                    ]
                });

                const deliveriesCollection = await pb.collections.getFirstListItem('name="bird_deliveries"');
                ...
                await createCollectionSafe({
                    ...
                    fields: [
                        { name: 'user', type: 'relation', required: true, collectionId: usersCollection.id, cascadeDelete: true, maxSelect: 1 },
                        { name: 'delivery', type: 'relation', required: true, collectionId: deliveriesCollection.id, cascadeDelete: true, maxSelect: 1 }
                    ]
                });


                try {
                    console.log("Creating 'public_bird_map' view collection...");
                    await pb.collections.create({
                        name: 'public_bird_map',
                        type: 'view',
                    ...
                    if (viewErr.data && viewErr.data.data && viewErr.data.data.name) {
                        console.log("-> 'public_bird_map' view already exists, updating...");
                        try {
                            const existingView = await pb.collections.getFirstListItem('name="public_bird_map"');
                            ...
                            console.error("FAILED to update public_bird_map view:", JSON.stringify(updateErr.data, null, 2));
                        }
                    } else {
                        console.error("FAILED to create public_bird_map view:", JSON.stringify(viewErr.data, null, 2));
                    }
                }
